...
for nEndPts=1:size(LFPEndPts,1)
    ...
    % spikeCells{nEndPts}=spikeIdx(nEndPts(nEndPts,1):nEndPts(nEndPts,2));
...
for myIter=1:nIter
    ...
    for nSegments=1:length(dataCells)
        ...
        elseif hilbertDim=="angle"
            randData=[randData,angle(hilbert(f_rotate))];
        else
    ...
    myProbability=histcounts(randData(spikeCells),myEdges,"Normalization","probability");
