...
deltaPowerCFS=abs(cfs(frq>=0.5 & frq<=4,:));
% thetaPowerThreshed=thetaPowerCFS(:,aboveThresh(1):aboveThresh(end));

deltaPowerRaw=sum(deltaPowerCFS,"all");
%% Spike repeats 3
...
deltaPower5Hz=sum(deltaPowerCFS,"all");
...
deltaPower200Hz=sum(deltaPowerCFS,"all");
% yline(thresh)
%% Spike repeats 1 
% spike_repeat=[zeros(1,1*fs),spike_cutout,zeros(1,0.2*fs),spike_cutout,zeros(1,0.2*fs),spike_cutout,zeros(1,1*fs)];
% t_rep=1/fs:1/fs:length(spike_repeat)/fs;
...
deltaPower1=sum(deltaPowerCFS,"all");
% yline(thresh)
...
bar(cats,[deltaPowerRaw,deltaPower1,deltaPower5Hz,deltaPower200Hz])
ax=gca;
ax.YScale="log";
ylim([10^4,10^8])
ax.FontSize=14;
