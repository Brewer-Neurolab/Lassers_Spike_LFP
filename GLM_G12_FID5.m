...
soma=load("D:\Brewer lab data\Slow Oscillation 4 Chamber 5 Tunnel Arrays\4x 210715 210806\1\Well Spikes\4x 33152 210715 21div 210806_1.h5\E12.mat");
soma=soma.data;

spikes=load("D:\Brewer lab data\Slow Oscillation 4 Chamber 5 Tunnel Arrays\4x 210715 210806\1\Well Spikes\4x 33152 210715 21div 210806_1.h5\E12_spikes.mat");
...
spike_dyn_tbl=load("D:\Brewer lab data\Slow Oscillation 4 Chamber 5 Tunnel Arrays\4x 210715 210806\1\Well Spikes\well_spike_dynamics_table_hfs.mat");
spike_dyn_tbl=spike_dyn_tbl.well_spike_dynamics_table;

burst_bounds=spike_dyn_tbl.BurstBounds{371};
burst_starts_idx=burst_bounds(:,1);

spike_train=zeros(1,length(t));
spike_train(burst_starts_idx)=1;
glmTbl=table();
spike_idx=round(remap(find(spike_train),1,length(t),1,300*re_fs));
spike_train=zeros(1,300*re_fs);
spike_train(spike_idx)=1;
...
%randomly choose samples equal to number of burst starts
rand_samples=randperm(length(spike_train));
rand_samples(ismembertol(rand_samples,find(spike_train)))=[];

ThetaAmp=ThetaAmp([find(spike_train),rand_samples(1:length(find(spike_train)))]);
ThetaAngle=ThetaAngle([find(spike_train),rand_samples(1:length(find(spike_train)))]);

glmTbl.WellSpikes=logical(spike_train([find(spike_train),rand_samples(1:length(find(spike_train)))])');
glmTbl.ThetaAmp=ThetaAmp';
% glmTbl.ThetaAngle=discretize(ThetaAngle',angle_edges,angle_edges(2:end));
glmTbl.ThetaAngle=ThetaAngle';
glmTbl.SinThetaAngle=sin(glmTbl.ThetaAngle);
...
mdl_burst_start=fitglm(glmTbl,modelspec,'Distribution','binomial','Link','logit');