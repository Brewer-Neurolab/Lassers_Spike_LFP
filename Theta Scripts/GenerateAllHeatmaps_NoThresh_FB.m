...
for fi=1:length(axon_spikes)
    for nelec=1:height(axon_spikes{fi})
        if ~isempty(axon_spikes{fi}.up_fb{nelec}) 
            fb_axon_tbl.fi(row)=fi;
            fb_axon_tbl.Subregion(row)=axon_spikes{fi}.Subregion(nelec);
            fb_axon_tbl.interRegi(row)=find(interRegions==axon_spikes{fi}.Subregion(nelec));
            electrodes=axon_spikes{fi}.("Electrode Pairs")(nelec);
            electrodes=split(electrodes,{'-'});
            fb_axon_tbl.Electrode(row)=electrodes(1);
            tunnelReg=split(axon_spikes{fi}.Subregion(nelec),{'-'});
            fb_axon_tbl.FBReg(row)=tunnelReg(1);
            fb_axon_tbl.subi(row)=find(subregions==tunnelReg(1));
...
%testing range
testing_idx=[find(fb_axon_tbl.Subregion=="CA3-CA1")]';

relationTable=[];

for nFF=1:height(fb_axon_tbl)
    data=load(fullfile(parent_axons_dir,axons_folders(fb_axon_tbl.fi(nFF)),fb_axon_tbl.Electrode(nFF)+".mat"));
    % re_fs=data.re_fs;
    re_fs=1000;
    fs=25000;
    ...
    % Regression tests
    targetElecs=well_spike_dyn.channel_name(well_spike_dyn.fi==fb_axon_tbl.fi(nFF) & well_spike_dyn.regi==fb_axon_tbl.subi(nFF));
    targetReg=subregions(well_spike_dyn.regi(well_spike_dyn.fi==fb_axon_tbl.fi(nFF) & well_spike_dyn.regi==fb_axon_tbl.subi(nFF)));
    sourceReg=fb_axon_tbl.Subregion(nFF);
    myTable=sourceLFP_targetSpike_relations_NoThresh(t,re_t,data,logicalValidLFPs,LFPEndPts,LFPAmplitude,LFPAngles,fb_axon_tbl.fi(nFF),fb_axon_tbl.Electrode(nFF),sourceReg,targetElecs,targetReg,well_spike_dyn,20,thresh_mult,...
        fullfile(parent_wells_dir,wells_folders(fb_axon_tbl.fi(nFF))+"\"),...
        "C:\Users\lasss\Documents\Research\Brewer Lab work\Code\Lassers_Spike_LFP\Images\Theta\Spikes No Thresh LogLog 5SD FB");
    ...
    disp(nFF+" of "+height(fb_axon_tbl))

    close all force
end

save(fullfile(saveDir,"relationTable_NoThresh_FB"),"relationTable")
%% Good Relationships
