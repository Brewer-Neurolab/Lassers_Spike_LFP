...
for nFF=1:height(ff_axon_tbl)%[95,96,117]
    ...
    for nWell=1:length(myWellElecs)
        ...
        for nBursts=1:size(burstBounds,1)
            burstStarts(burstBounds(nBursts,1))=1;
        end
        glmTbl=table();
        burstTrainIdx=round(remap(find(burstStarts),1,length(t),1,300*re_fs));
        burstStarts=zeros(1,length(re_t));
        burstStarts(burstTrainIdx)=1;
        burstStarts(~logicalValidLFPs)=[];
        
        glmTbl.WellBursts=logical(burstStarts');
