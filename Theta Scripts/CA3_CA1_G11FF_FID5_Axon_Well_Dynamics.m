...
data=load("D:\Brewer lab data\Slow Oscillation 4 Chamber 5 Tunnel Arrays\4x 210715 210806\1\downsampled tunnels\theta\4x 33152 210715 21div 210806_1_mat_files\G11.mat");
re_fs=data.re_fs;
...
for nElec=1:length(CA3_Electrodes)
    ...
    well_burst_bounds=well_spike_dyn.BurstBounds{well_spike_dyn.fi==5 & well_spike_dyn.channel_name==myElec};
    well_burst_starts=well_burst_bounds(:,1);
    % remap burst starts to new sampling rate
    well_burst_starts=round(remap(well_burst_starts,1,length(t),1,length(re_t)));
    logicalBurstStarts=zeros(1,length(re_t));
...
seAmpProbs=sdAmpProbs./sqrt(size(ampProbs,1));
...
MI_tbl=cummulative_axon_well_burst_start(t,re_t,logicalValidLFPs,LFPAmplitude,LFPAngles,5,"G11",targetElecs,well_spike_dyn,40,thresh_mult);
%% scatter plot SPB vs Burst Length
scatter_BL_v_SPB(well_spike_dyn,MI_tbl(MI_tbl.pval~="NA",:),t,re_t,logicalValidLFPs)
%% Regression tests
targetElecs=well_spike_dyn.channel_name(well_spike_dyn.fi==5 & well_spike_dyn.regi==4);
sourceLFP_targetSpike_relations(t,re_t,logicalValidLFPs,LFPAmplitude,LFPAngles,5,"G11",targetElecs,well_spike_dyn,20,thresh_mult,...
...
figure
%NOT logical valid lfps means everywhere outside of high axonal amps
wellBurstStartAngles=LFPAngles(logicalBurstStarts & ~logicalValidLFPs);
% wellBurstStartAngles=[wellBurstStartAngles-360,wellBurstStartAngles];
wellBurstStartAmp=LFPAmplitude(logicalBurstStarts & ~logicalValidLFPs);
...
X=[repwellBurstStartAngles;repwellBurstStartAmp]';
edges={[0:40:360],logspace(log10(0.01),log10(max(LFPAmplitude)),10)};
...
xlim([0,360])
yticks(logspace(log10(0.01),log10(max(LFPAmplitude)),10))
ylim([0.01,max(LFPAmplitude)])
...
v=VideoWriter("C:\Users\lasss\Documents\Research\Brewer Lab work\Code\Lassers_Spike_LFP\Test Images\M9 Phase Amp Stack.mp4");
v.FrameRate=2;
open(v)
figure
set(gca,"NextPlot","replacechildren")
...
for nElec=1:length(CA3_Electrodes)
    %find burst starts
    well_burst_bounds=well_spike_dyn.BurstBounds{well_spike_dyn.fi==5 & well_spike_dyn.channel_name==CA3_Electrodes(nElec)};
    well_burst_starts=well_burst_bounds(:,1);
    % remap burst starts to new sampling rate
    well_burst_starts=round(remap(well_burst_starts,1,length(t),1,length(re_t)));
    logicalBurstStarts=zeros(1,length(re_t));
    ...
    % get spikes from channel
    spikes=load("D:\Brewer lab data\Slow Oscillation 4 Chamber 5 Tunnel Arrays\4x 210715 210806\1\Well Spikes\4x 33168 210715 21div 210806_1.h5\"+CA3_Electrodes(nElec)+"_spikes.mat");
    spikes=spikes.index/1000;
    fs=25000;
    t=0:1/fs:t_rec-(1/fs);
    spikes=ismembertol(t,spikes,1e-10);
...
%% Out of sync target for control 
