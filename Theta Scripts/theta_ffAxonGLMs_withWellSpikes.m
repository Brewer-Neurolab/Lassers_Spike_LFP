...
for nFF=1:height(ff_axon_tbl)%[95,96,117]
    ...
    [LFPEndPts,LFPAmplitude,LFPHilbert]=glm_identify_lfps(theta,re_fs,t_rec,thresh_mult,minLFPLength,minLFPCycles,nsamples_combine_thresh);
    ...
    for nWell=1:length(myWellElecs)
        ...
        glmTbl.ThetaAngle=zscore(ThetaAngle'); %toggle to zscore
        glmTbl.SinThetaAngle=zscore(sin(glmTbl.ThetaAngle)); %toggle to zscore
        glmTbl.CosThetaAngle=zscore(cos(glmTbl.ThetaAngle)); %toggle to zscore
...
for subi=1:length(all_reg)
    [myxlim,myylim]=glmScatter(glmTblAll,"mdl",2,all_reg(subi),0.05);
    ax=[ax,gca];

    if myxlim{1}(1)<minXLim
        minXLim=myxlim{1}(1);
    end
    if myxlim{1}(2)>maxXLim
        maxXLim=myxlim{1}(2);
    end
    if myylim{1}(1)<minYLim
        minYLim=myylim{1}(1);
    end
    if myylim{1}(2)>maxYLim
        maxYLim=myylim{1}(2);
    end
end

for i=1:length(ax)
    ax(i).XLim=[minXLim,maxXLim];
...
for subi=1:length(all_reg)
    [myxlim,myylim]=glmScatter(glmTblAll,"mdl",3,all_reg(subi),0.05);
    ax=[ax,gca];
...
for subi=1:length(all_reg)
    [myxlim,myylim]=glmScatter(glmTblAll,4,5,all_reg(subi),0.05);
    ax=[ax,gca];
...
for subi=1:length(all_reg)
    [myxlim,myylim]=glmScatter(glmTblAll,2,3,all_reg(subi),0.05);
    ax=[ax,gca];
...
for subi=1:length(all_reg)
    [myxlim,myylim]=glmScatter(glmTblAll,"mdl",4,all_reg(subi),0.05);
    ax=[ax,gca];

    ylabel("-log_{10} P (Phase-Amplitude)")
