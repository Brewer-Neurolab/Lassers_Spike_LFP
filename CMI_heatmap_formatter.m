function miTable=CMI_heatmap_formatter(jpxyz,jpxz,jpyz,bincounts_cells,binxcenters,binxedges,...
...
for nElec=1:length(bincounts_cells)
    ...
    if ~isempty(bincounts_cells{nElec}) && sum(bincounts_cells{nElec},"all")*2>=numel(binxcenters{nElec})*numel(binycenters{nElec})
        ...
        pval=shuffleMultivariateMutInfo(myMutInfo,sourceProps.LFPAmps,sourceProps.LFPAngles,sourceProps.logicalValidLFPs,...
        ...
        if pval<1/sourceProps.nIter
            title(targetElecs(nElec)+" MI="+round(myMutInfo,2,"significant")+" p<"+1/sourceProps.nIter+" n="+nBursts(nElec),"FontSize",12)
        else
            title(targetElecs(nElec)+" MI="+round(myMutInfo,2,"significant")+" p="+pval+" n="+nBursts(nElec),"FontSize",12)
        end
        set(gca,"FontSize",18)
